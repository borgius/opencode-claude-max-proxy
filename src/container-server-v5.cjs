...
log('INFO', 'Starting container server v5 (persistent process)', { host: HOST, port: PORT });
...
class ClaudeProcessManager {
  ...
  async ensureProcess() {
    if (this.process && !this.process.killed) {
      log('DEBUG', 'Reusing existing Claude process', { pid: this.process.pid, sessionId: this.sessionId });
      return;
    }

    log('INFO', 'Starting new persistent Claude process');
    ...
    log('INFO', 'Claude process spawned', { pid: this.process.pid, sessionId: this.sessionId, elapsed: Date.now() - startTime });
    ...
    this.process.on('close', (code) => {
      log('INFO', 'Claude process closed', { code, sessionId: this.sessionId });
      this.process = null;
      this.readline = null;
      this.isReady = false;
    ...
    // Don't wait for system init - just mark as ready immediately
    // The first message we send will trigger init, and we'll capture sessionId then
    this.isReady = true;
    log('INFO', 'Claude process ready (async init)', { elapsed: Date.now() - startTime, sessionId: this.sessionId });
  }

  handleMessage(msg) {
    this.lastActivity = Date.now();

    // Check for system init
    if (msg.type === 'system' && msg.session_id) {
      this.sessionId = msg.session_id;
      this.isReady = true;
      log('INFO', 'System init received', { sessionId: this.sessionId });
    }
    ...
    if (this.pendingRequest) {
      ...
      if (msg.type === 'result') {
        log('DEBUG', 'Result received, request complete');
        const req = this.pendingRequest;
        this.pendingRequest = null;
        req.onDone(0, this.sessionId);
  ...
  sendMessage(prompt, onEvent, onError, onDone) {
    ...
    if (this.pendingRequest) {
      // Queue the request
      log('DEBUG', 'Queueing request, current request in progress');
  ...
  async executeRequest(request) {
    ...
    try {
      ...
      log('DEBUG', 'Sending message', { requestId, promptLen: prompt.length });
  ...
  shutdown() {
    if (this.process) {
      log('INFO', 'Shutting down Claude process');
      this.process.kill();
      this.process = null;
    }
  }
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === '/v1/sessions' && req.method === 'POST') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({
      session_id: claudeManager.sessionId || 'not-started',
      note: 'This server uses a persistent session'
    }));
    return;
  }
  ...
  if (req.method === 'POST' && req.url.startsWith('/v1/messages')) {
    ...
    req.on('end', async () => {
      try {
        ...
        log('INFO', 'Processing', { reqId, msgId, stream, sessionId: claudeManager.sessionId });
        ...
        if (stream) {
          ...
          claudeManager.sendMessage(
              ...
              if (msg.type === 'stream_event' && msg.event) {
                ...
                if (evt.type === 'message_start' && !sentMessageStart) {
                  ...
                  res.write(`event: message_start\ndata: ${JSON.stringify({
                    ...
                    session_id: claudeManager.sessionId
                  })}\n\n`);
                }
            ...
            (code, sessionId) => {
              log('INFO', 'Stream done', { reqId, elapsed: Date.now() - reqStart, sessionId });
              res.end();
            }
          );
          ...
          claudeManager.sendMessage(
            ...
            (code, sessionId) => {
              log('INFO', 'Non-stream done', { reqId, elapsed: Date.now() - reqStart, sessionId });
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({
                id: msgId,
                type: 'message',
...
server.listen(PORT, HOST, () => {
  ...
  log('INFO', 'Features: persistent process, session reuse');
});
